    ...
    <div className={cn("flex flex-col", className)}>
                  ...
                  {file.errors
                    .map((e) =>
                      e.message.startsWith("File is larger than")
                        ? `الملف أكبر من ${formatBytes(maxFileSize, 2)} (الحجم: ${formatBytes(file.size, 2)})`
                        : e.message
                    )
                    .join(", ")}
                </p>
      ...
      {exceedMaxFiles && (
        <p className="text-sm text-left mt-2 text-destructive">
          يمكنك رفع {maxFiles} ملف فقط، يرجى إزالة{" "}
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-y-2", className)}>
      ...
      <p className="text-sm">
        ارفع {!!maxFiles && maxFiles > 1 ? ` ${maxFiles}` : ""} ملف
        {!maxFiles || maxFiles > 1 ? "" : ""}
      </p>
      <div className="flex flex-col items-center gap-y-1">
        <p className="text-xs text-muted-foreground">
          ...
          >
            اختر {maxFiles === 1 ? `ملف` : "ملفات"}
          </a>{" "}
          للرفع
        </p>
        {maxFileSize !== Number.POSITIVE_INFINITY && (
          <p className="text-xs text-muted-foreground">
            الحد الأقصى لحجم الملف: {formatBytes(maxFileSize, 2)}
          </p>
        )}
      </div>
    </div>
